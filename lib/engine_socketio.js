...
function isAcknowledgementRequired(spec) {
  return (spec.emit && spec.emit.acknowledgement);
}
...
SocketIoEngine.prototype.step = function (requestSpec, ee) {
  ...
  let f = function(context, callback) {
    ...
    let acknowledgementDone = false;

    let responseError = false;
    let acknowledgementError = false;

    let emit = function( channel, data ) {
      if (isAcknowledgementRequired(requestSpec)) {
        let response = {
          capture: template(requestSpec.emit.acknowledgement.capture, context),
          match: template(requestSpec.emit.acknowledgement.match, context)
        };
        socketio.emit(channel, data, function(acknowledgement) {
          processResponse(ee, acknowledgement, response, context, function(err) {
            acknowledgementDone = true;
            if( err ) {
              // always end request in case of an error
              acknowledgementError = true;
              return callback(err, context);
            } else if ((!isResponseRequired(requestSpec) || responseDone) && !responseError) {
              // do not end if there's an outstanding response, or if we already ended the request due to an error in the response
      ...
      if (isResponseRequired(requestSpec) || isAcknowledgementRequired(requestSpec)) {
        ...
        setTimeout(function responseTimeout() {
            if (isResponseRequired(requestSpec) && !responseDone ||
              isAcknowledgementRequired(requestSpec) && !acknowledgementDone) {
    ...
    if (isResponseRequired(requestSpec)) {
      ...
      socketio.on(response.channel, function receive(data) {
        processResponse(ee, data, response, context, function(err) {
          ...
          if( err ) {
            ...
            responseError = true;
            return callback(err, context);
          } else if ((!isAcknowledgementRequired(requestSpec) || acknowledgementDone) && !acknowledgementError) {
            // if there is no acknowledgement required or if we already received it without error, we're done and can end the request
            markEndTime(ee, context, startedAt);
            return callback(err, context);
          }
        });
      ...
      emit(outgoing.channel, outgoing.data);
